...
const uint8_t convTable_[] PROGMEM = {0x40, 0x79, 0x24, 0x30, 0x19, 0x12, 0x02,
		0x78, 0x00, 0x18, 0x08, 0x03, 0x46, 0x21, 0x06, 0x0E};

/* Tabela de conversão em flash: Catodo comum */
#ifdef COM_CATODO
const uint8_t convTable_[] PROGMEM = {0x3F, 0x06, 0x5B, 0x4F, 0x66, 0x6D, 0x7D,
...
void displaysWrite(uint8_t hexValue){
	...
	for (i = 0; i < displays_info.total; i++){
		...
		data = (0x0f & hexValue);
		data = pgm_read_byte(&convTable_[data]);
